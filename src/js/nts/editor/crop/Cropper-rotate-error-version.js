...
export class Cropper extends PIXI.Container {
    ...
    resizeImage() {
        ...
        this.resizeUIPoints = this.image.points;
    }
    ...
    rotateChange(e) {
        ...
        if(this.isImageOutOfBounds) {
            var rotationPoints = Calc.getRotationRectanglePoints({x:this.image.x, y:this.image.y}, this.resizeUIPoints, Calc.toDegrees(this.image.rotation));
    ...
    cornerResizeChange(e) {
        ...
        if (tx > this.startLensBounds.x && tx < (this.startLensBounds.x + this.startLensBounds.width) && ty > this.startLensBounds.y && ty < (this.startLensBounds.y + this.startLensBounds.height)) {
            target.x = tx;
            target.y = ty;
            this.resizeUI.updateOtherCorner(target);
        } else {
            if(tx < lens.x) {
                isOutX = true;
                lens.x = lens.x - dx;
